...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_CHAR :
            // 键盘字符输入事件
            if (fBlocking && (wParam == '\x1B')) // 如果正在绘制矩形框且按下的是 Escape 键
            {
...
在 WM_LBUTTONDOWN 消息中，当鼠标左键按下时，记录鼠标位置并绘制初始矩形框。同时，通过调用 SetCapture 函数捕捉鼠标，将鼠标光标设置为十字形。
在 WM_MOUSEMOVE 消息中，当鼠标移动时，如果拦截标志为真，则设置鼠标光标为十字形，并擦除前一次绘制的矩形框，更新绘制新的矩形框。
在 WM_LBUTTONUP 消息中，当鼠标左键松开时，如果拦截标志为真，则擦除最后一次绘制的矩形框，并记录最终的矩形框的坐标。然后，通过调用 ReleaseCapture 函数释放鼠标，将鼠标光标设置为箭头形状，取消拦截标志，将有效矩形标志设置为真，并无效化窗口的矩形区域以便重绘。
在 WM_CHAR 消息中，当键盘输入字符时，如果拦截标志为真且按下的是 Escape 键，将擦除当前绘制的矩形框，并通过调用 ReleaseCapture 函数释放鼠标，将鼠标光标设置为箭头形状，取消拦截标志。
...
这段代码实现了一个可绘制矩形框的窗口，并在鼠标操作和键盘输入时实时更新绘制效果。它通过记录鼠标的位置和状态来跟踪矩形框的绘制过程，并在需要时使用特定的绘图函数来更新和擦除矩形框。同时，它使用了 SetCapture 和 ReleaseCapture 函数来捕捉和释放鼠标，确保在拖动绘制矩形框时鼠标操作的正确性。
 */